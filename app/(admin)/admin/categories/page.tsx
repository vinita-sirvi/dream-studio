    ...
    <main className="grid gap-6">
      <section className="rounded-[2rem] border border-[#dce4e3] bg-white/85 p-8 shadow-[0_18px_42px_rgba(15,30,30,0.08)]">
        <div className="flex items-center justify-between gap-4">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.28em] text-[#b23a17]">Admin / Categories</p>
            <h1 className="mt-3 text-4xl font-medium text-[#0f1e1e]" style={{ fontFamily: "Georgia, 'Times New Roman', serif" }}>
              Category management
            </h1>
          </div>
          <Link href="/admin/categories/new" className="rounded-md bg-[#0b1717] px-5 py-3 text-sm font-semibold uppercase tracking-[0.14em] text-white">
            New Category
          </Link>
        </div>
      </section>
        ...
        <table className="w-full text-left text-sm">
          ...
          <tbody>
            {categories.map((category: any) => (
              <tr key={String(category._id)} className="border-t border-[#dce4e3]">
                ...
                <td className="px-4 py-4 flex gap-2">
                  <Link href={`/admin/categories/${String(category._id)}`} className="rounded-md border border-[#c3cfce] bg-white px-3 py-2 text-xs font-semibold uppercase tracking-[0.14em] text-[#0b1717]">
                    Edit
                  </Link>
                  <AdminDeleteButton resource="categories" id={String(category._id)} />
                </td>
              </tr>
            ))}
          </tbody>
