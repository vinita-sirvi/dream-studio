    ...
    <main className="grid gap-6">
      <section className="rounded-[2rem] border border-[#eadccc] bg-white/85 p-8 shadow-[0_18px_42px_rgba(103,73,47,0.08)]">
        <div className="flex items-center justify-between gap-4">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.28em] text-[#8a6b56]">Admin / Categories</p>
            <h1 className="mt-3 text-4xl font-medium text-[#2f2319]" style={{ fontFamily: "Georgia, 'Times New Roman', serif" }}>
              Category management
            </h1>
          </div>
          <Link href="/admin/categories/new" className="rounded-md bg-[#3b2417] px-5 py-3 text-sm font-semibold uppercase tracking-[0.14em] text-white">
            New Category
          </Link>
        </div>
      </section>
        ...
        <table className="w-full text-left text-sm">
          ...
          <tbody>
            {categories.map((category: any) => (
              <tr key={String(category._id)} className="border-t border-[#f0e3d6]">
                ...
                <td className="px-4 py-4 flex gap-2">
                  <Link href={`/admin/categories/${String(category._id)}`} className="rounded-md border border-[#d8c5b0] bg-white px-3 py-2 text-xs font-semibold uppercase tracking-[0.14em] text-[#3b2417]">
                    Edit
                  </Link>
